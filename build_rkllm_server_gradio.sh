...
#*****************************************************************************************#
# RKLLM-Server-Gradio Build Script
...
OPTIONS:
  ...
  --port PORT             Server port for local mode (default: 7860)
...
if [[ "$LOCAL_MODE" == "true" ]]; then
    ...
    if ! python3 -c "import gradio" &> /dev/null; then
        echo "⚠️  Gradio not installed. Installing..."
...
if ! python3 -c "import gradio" 2>/dev/null; then
    echo "⚠️  Installing Gradio..."
    pip3 install gradio>=4.24.0 -i https://pypi.tuna.tsinghua.edu.cn/simple/ --break-system-packages 2>/dev/null || pip3 install gradio>=4.24.0
else
    echo "✅ Gradio is installed"
fi
BOARD_EOF
    ...
    # Start the server
    echo "\n🚀 Starting Gradio server on board..."
    echo "========================================"
