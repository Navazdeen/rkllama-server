...
from rkllm_server.db.chat_database import ChatDatabase, getChatDatabase
...
thinking_updates = {"current": [], "lock": threading.Lock()}  # Current thinking updates
...
def initializeModel(model_path: str, platform: str, model_id: str = "qwen") -> bool:
...
def initializeModelManager(model_folder: str, platform: str) -> bool:
    ...
    try:
        ...
        if initializeModel(first_model_path, platform, first_model):
...
def pull_model_from_hf(
    hf_url: str, model_name: Optional[str] = None
) -> Tuple[bool, str]:
...
def summarize_history(history: List[Dict]) -> List[Dict]:
    ...
    summary_text = (
        f"[📝 Conversation Summary: {len(old_messages)} previous messages discussing "
    )
...
def generate_title_with_model(user_message: str) -> str:
    ...
    if rkllm_model is None or not user_message:
        ...
        return ChatDatabase.generateTitleFromMessage(
            user_message, model_name or "RKLLM"
        )
...
Title:"""
            ...
            model_thread = threading.Thread(
                target=rkllm_model.run, args=("user", False, title_prompt)
            )
            model_thread.start()
            ...
            if title_output.strip():
                title = ChatDatabase.extractSummaryFromModelResponse(
                    title_output.strip()
                )
                if title:
        ...
        if role == "user" and current_chat_id not in title_updated_for_chat:
            ...
            if chat_info:
                ...
                if new_title:
                    ...
                    current_auto_title = chat_db._generateTitleFromModel(
                        model_name or "RKLLM"
                    )
                    if new_title != current_auto_title:
                        ...
                        print(
                            f"✅ Chat title updated from '{current_auto_title}' to: '{new_title}'"
                        )
...
def generate_response(
    prompt: str, history: Optional[List[Dict]] = None, stream: bool = True
) -> str:
    ...
    try:
        with lock:
            ...
            model_thread = threading.Thread(
                target=rkllm_model.run, args=("user", False, full_prompt)
            )
            model_thread.start()
...
def createGradioInterface():
    ...
    with gr.Blocks(
        title=f"RKLLM Chat - {model_display}",
        ...
        **{model_display.upper()}** • {platform_display.upper()}
        ...
        with gr.Row(equal_height=False):
            ...
            with gr.Column(scale=1, elem_classes="left-sidebar"):
                ...
                display_choices = [
                    f"{cid[:4]}... - {chat_titles.get(cid, 'Chat')}"
            ...
            with gr.Column(scale=1, elem_classes="right-sidebar"):
                ...
                # Model settings
                stream_toggle = gr.Checkbox(label="🌊 Streaming", value=True)

                context_toggle = gr.Checkbox(label="🧠 Use Context", value=True)
                ...
                apply_config_btn = gr.Button(
                    "💾 Save Config", scale=1, size="sm", variant="secondary"
                )
                ...
                switch_model_btn = gr.Button(
                    "🔀 Load Model", scale=1, variant="primary"
                )
                ...
                with gr.Row():
                    ...
                    hf_pull_btn = gr.Button(
                        "🔽 Pull", scale=1, variant="primary", size="sm"
                    )
        ...
        def get_dropdown_choices():
            ...
            chat_titles = get_chat_titles_dict()
            return [
                f"{cid[:4]}... - {chat_titles.get(cid, 'Chat')}" for cid in chat_ids
            ]
        ...
        def on_search_chats(query: str):
            ...
            return gr.Dropdown(
                choices=choices if choices else ["No matches"],
                value=choices[0] if choices else "No matches",
            )
        ...
        def on_switch_session_by_display(display_text):
            ...
            try:
                ...
                full_chat_id = next(
                    (cid for cid in all_chats if cid.startswith(chat_id_prefix)), None
                )
        ...
        def on_delete_session_by_display(display_text):
            ...
            if not display_text or display_text == "New Chat":
                return (
                    gr.Dropdown(choices=get_dropdown_choices(), value="New Chat"),
                    [],
                    update_session_info(),
                    update_session_list_display(),
                    "❌ No chat selected",
            ...
            return (
                gr.Dropdown(
                    choices=choices, value=choices[0] if choices else "New Chat"
                ),
                [],
                update_session_info(),
                update_session_list_display(),
        ...
        def format_response_with_search_and_thinking(
            response: str,
        ...
        def respond(
                ...
                try:
                    ...
                    if use_thinking:
                        ...
                        loop_thinking_results = loop_engine.gather_information_loop(
                            query=optimized_query, search_func=loop_search
                        )
                        ...
                        if loop_thinking_results["gathered_info"]:
                            update_thinking_display(
                                f"✅ Information gathering complete in {loop_thinking_results['iterations']} iterations\n"
                            )
                            ...
                            print(
                                f"✅ Gathered info in {loop_thinking_results['iterations']} iterations"
                            )
                    ...
                    # Step 3: Regular search if not using loop thinking
                        ...
                        update_thinking_display(
                            f"🌐 Searching: '{optimized_query}' (max {search_config['max_results']} results)\n"
                        )
                        search_results = search_web(
                            optimized_query, max_results=search_config["max_results"]
                        )
                        if search_results:
                            # Enhance results with content extraction
                            search_results = ContentExtractor.extract_from_results(
                                search_results
                            )
                            ...
                            update_thinking_display(
                                f"✅ Web search complete: {len(search_results)} results found\n"
                            )
                            augmented_message = search_context + "\n\n" + message
                            ...
                            print(
                                f"🔍 Web search found {len(search_results)} results with content"
                            )
            ...
            if use_thinking and not loop_thinking_results:
                try:
                    augmented_message = inject_thinking(
                        augmented_message, pattern="chain_of_thought"
                    )
                    ...
                    for partial_response in generate_response_streaming(
                        ...
                        if loop_thinking_results:
                            thinking_steps = "\n".join(
                                [
                                    f"  • {s}"
                                    for s in loop_thinking_results["thinking_steps"]
                                ]
                            )
                        ...
                        if (
                            updated_history
                            and updated_history[-1]["role"] == "assistant"
                        ):
...
"""
        ...
        chat_search.change(
            on_search_chats, inputs=[chat_search], outputs=[session_dropdown]
        )
        ...
        def apply_config_and_update(n_iter: int, max_res: int, info_len: int):
            ...
            print(
                f"✅ Configuration updated: iterations={n_iter}, results={max_res}, threshold={info_len}"
            )
        ...
        def on_switch_model(selected_model):
            ...
            try:
                ...
                if not model_path:
                    return (
                        f"❌ Model path not found: {selected_model}",
                ...
                with lock:
                    ...
                    # Load new model
                    if initializeModel(model_path, target_platform, selected_model):
                        print(f"✅ Successfully switched to: {selected_model}")
                        return (
                            f"✅ Switched to: {selected_model}",
                            get_model_info_detailed(),
                        )
                    else:
                        return (
        ...
        def on_hf_pull(hf_url: str, custom_name: str):
            ...
            try:
                ...
                model_name_for_pull = (
                    custom_name if custom_name and custom_name.strip() else None
                )
                ...
                try:
                    ...
                    for line in process.stdout:
                        ...
                        if line_strip and any(
                            x in line_strip
                            for x in ["Receiving", "Resolving", "Unpacking", "objects"]
                        ):
        ...
        ).then(refresh_model_selector, outputs=[model_selector])

        # Event handler for actions
        clear_btn.click(lambda: [], None, chatbot).then(
            reset_chat_and_update, outputs=[session_info]
        )

        model_info_btn.click(get_model_info_detailed, outputs=[model_info_display])
            ...
            if not chat_ids:
                ...
                if chat_db:
                    ...
                    if chats:
                        ...
                        current_session_id = (
                            chat_ids[0] if chat_ids else create_new_session()
                        )
                        current_chat_id = current_session_id
                else:
                    current_session_id = (
